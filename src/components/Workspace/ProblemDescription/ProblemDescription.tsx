...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({ problem, _solved }) => {
    ...
    const handleStar = async () => {
        if (!user) {
            toast.error('you must be loegged in to star a problem', { position: 'top-left', theme: 'dark' })
            return
        }
        if (updating) return
        setUpdating(true)
                    ...
                    <div className='w-full'>
                        ...
                        {!loading && currentProblem && (
                            <div className='flex items-center mt-3'>
                                ...
                                <div className='flex items-center cursor-pointer hover:bg-dark-fill-3 space-x-1 rounded p-[3px]  ml-4 text-lg
                                transition-colors duration-200 text-dark-gray-6' onClick={handleLike}>
                                    {liked && !updating && <AiFillLike className="text-dark-blue-s" />}
                                    {!liked && !updating && <AiFillLike></AiFillLike>}
                                    {updating && <AiOutlineLoading3Quarters className="animate-spin"></AiOutlineLoading3Quarters>}
                                    <span className='text-xs'>{currentProblem.likes}</span>
                                </div>
                                <div className='flex items-center cursor-pointer hover:bg-dark-fill-3 space-x-1 rounded p-[3px]  ml-4 text-lg transition-colors
                                duration-200 text-green-s text-dark-gray-6' onClick={handleDislike}>
                                    {disliked && !updating && <AiFillDislike className="text-dark-blue-s"></AiFillDislike>}
                                    {!disliked && !updating && <AiFillDislike></AiFillDislike>}
                                    {updating && <AiOutlineLoading3Quarters className="animate-spin"></AiOutlineLoading3Quarters>}
                                    <span className='text-xs'>{currentProblem.dislikes}</span>
                                </div>
                                <div className='cursor-pointer hover:bg-dark-fill-3  rounded p-[3px]  ml-4 text-xl transition-colors duration-200 text-green-s text-dark-gray-6'
                                    onClick={handleStar}>
                                    {starred && !updating && <AiFillStar className="text-dark-yellow" ></AiFillStar>}
                                    {!starred && !updating && <TiStarOutline></TiStarOutline>}
                                    {updating && <AiOutlineLoading3Quarters className="animate-spin"></AiOutlineLoading3Quarters>}
                                </div>
                            </div>
                        )}
                        ...
                        <div className='mt-4'>
                            {problem.examples.map((example, index) => (
                                <div key={example.id}>
                                    ...
                                    {example.img && (
                                        <img src={example.img} className="mt-3"></img>
                                    )}
